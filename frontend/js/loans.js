// Admin-only active loans page. Depends on api.js + nav.js.
...
function renderLoansTable(loans) {
  ...
  if (!loans.length) {
    container.innerHTML = `<div class="empty-state">No loans found.</div>`;
    return;
  }
      ...
      <tbody>
        ${loans
          ...
          <tr>
            ...
            <td>${escapeHtml(loan.user?.name || "Unknown user")}</td>
            ...
            <td>
              ${
                loan.returnedAt
                  ? ""
                  : `<button type="button" class="btn btn-sm" data-loan-id="${loan._id}">Mark Returned</button>`
              }
            </td>
          </tr>
        `
  ...
  container.querySelectorAll("button[data-loan-id]").forEach((btn) => {
    btn.addEventListener("click", () => handleMarkReturned(btn.dataset.loanId));
  });
}
...
document.addEventListener("DOMContentLoaded", () => {
  if (document.getElementById("loans-table-container")) initLoansPage();
});
